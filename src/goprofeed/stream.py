...
def gopro_live():
    ...
    if "HD4" in response or "HD3.2" in response or "HD5" in response or "HX" in response or "HD6" in response or "HD7" in response:
        ...
        if SAVE == False:
            if STREAM == True:
                subprocess.Popen(
                    "ffmpeg " + loglevel_verbose +
                    " -fflags nobuffer -f:v mpegts -probesize 8192 -i udp://10.5.5.100:8554 -f mpegts -vcodec copy udp://localhost:10000",
                    shell=True
                )
            else:
                subprocess.Popen(
                    "ffplay " + loglevel_verbose +
                    " -fflags nobuffer -f:v mpegts -probesize 8192 udp://10.5.5.100:8554",
                    shell=True
                )
        else:
            if SAVE_FORMAT == "ts":
            ...
            subprocess.Popen(
                'ffmpeg -i "udp://10.5.5.100:8554" -fflags nobuffer -f:v mpegts -probesize 8192 ' + TS_PARAMS +
                SAVELOCATION,
                shell=True
            )
